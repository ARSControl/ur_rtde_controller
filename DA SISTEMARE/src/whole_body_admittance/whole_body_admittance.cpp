...
//	while(mir_callback_flag == false){ros::spinOnce();};
	...
	ur_joint_position = ur_controller.get_joint_position();
		std::cout << ur_joint_position << std::endl;

	Eigen::Matrix<double, 7, 1> whole_body_joint_position;
	Eigen::Matrix<double, 8, 1> whole_body_joint_velocity = Eigen::Matrix<double, 8, 1>::Zero(8,1);
	Eigen::Matrix<double, 8, 1> whole_body_joint_velocity_old = Eigen::Matrix<double, 8, 1>::Zero(8,1);
	...
	Eigen::Matrix<double, 4, 4> T_ur_wrt_ur_base;
	Eigen::Matrix<double, 4, 4> T_ur_wrt_mir;

	Eigen::Matrix<double, 3, 1> safety_sphere_centre;
	safety_sphere_centre << -0.2, 0.0, 1.3;
	double safety_sphere_radius = 0.25;
	bool on_edge = false;
//	double distance, reduction_factor;

//	Eigen::Matrix<double, 8, 1> external_torque;
...
//	Eigen::Matrix<double, 2, 1> v_mir;
	...
	while (ros::ok()){
		...
		T_ur_wrt_ur_base = ur_controller.get_pose();
		T_ur_wrt_mir = T_ur_base_wrt_mir * T_ur_wrt_ur_base;
...
//		distance = ((T_ur_wrt_mir.block<3,1>(0,3) - safety_sphere_centre).norm() - safety_sphere_radius);
		if(((T_ur_wrt_mir.block<3,1>(0,3) - safety_sphere_centre).norm() - safety_sphere_radius) > 0.0){
			if(external_force.block<3,1>(0,0).norm()>0.0){
				if(external_force.block<3,1>(0,0).dot((safety_sphere_centre - T_ur_wrt_mir.block<3,1>(0,3)).normalized()) < 0.0){
	//				external_torque = whole_body_jacobian.transpose() * external_force;
	//				external_torque.block
	//			reduction_factor = 1.0 - 1.0/0.05 * (distance);
	//			if(reduction_factor < 0.1)
	//				reduction_factor = 0.1;
					whole_body_jacobian.block<6,6>(0,2) =  Eigen::Matrix<double, 6, 6>::Zero(6,6);
					on_edge = true;
				}
				else{
					on_edge = false;
...
//		external_force(2,0) = 0.0;
		...
		if (on_edge == true)
			whole_body_cartesian_velocity(2,0) = 0.0;

		whole_body_joint_velocity_old = whole_body_joint_velocity;
		whole_body_joint_velocity = whole_body_jacobian.transpose() * (whole_body_jacobian * whole_body_jacobian.transpose() +
									1e-6 * Eigen::Matrix<double, 6, 6>::Identity(6,6)).inverse() * whole_body_cartesian_velocity;

		whole_body_joint_velocity(0,0) = low_pass_filter(whole_body_joint_velocity(0,0), whole_body_joint_velocity_old(0,0), 0.002, 20.0);
		whole_body_joint_velocity(1,0) = low_pass_filter(whole_body_joint_velocity(1,0), whole_body_joint_velocity_old(1,0), 0.002, 20.0);
		for(int i=2; i<8; i++)
			whole_body_joint_velocity(i,0) = low_pass_filter(whole_body_joint_velocity(i,0), whole_body_joint_velocity_old(i,0), 0.002, 25.0);
...
//		v_mir << 0.0,0.05;
//		v_mir = T_osfl * v_mir;
		mir_cmd.linear.x = whole_body_joint_velocity(0,0);
		mir_cmd.angular.z = - whole_body_joint_velocity(1,0);
...
//		std::cout << std::endl << "ur speed:" << std::endl << whole_body_joint_velocity << std::endl;
		...
		std::cout << std::endl << whole_body_joint_velocity << std::endl << std::endl;
